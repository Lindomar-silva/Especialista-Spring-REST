...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<?> handleEntidadeNaoEncontrada(AccessDeniedException ex, WebRequest request) {
		...
		Problem problem = getProblem(status, ProblemType.ACESSO_NEGADO, datail);
		problem.setUserMsg(datail);
		problem.setUserMsg("Você não possui permissão para executar essa operação.");
	...
	private Problem getProblem(HttpStatusCode status, ProblemType problemType, String datail) {
		Problem problem = new Problem();
		problem.setStatus(status.value());
